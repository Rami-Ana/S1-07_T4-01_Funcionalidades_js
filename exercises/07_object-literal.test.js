test('07_object-literal-1: can use shorthand for property names', () => {
  const  createMonster = (name, power) => {
    ...
    return {
      type: 'Monster',
      name, //shorthand, antes propiedad: variable = name: name,
      power, //shorthand antes propiedad: variable =  power: power,
      attack(target){
...
//  createMonster(name, power)
  ...
  expect(godzilla.attack(mechaGodzilla)).toBe('Godzilla attacked MechaGodzilla') //attack metodo
})
...
test('07_object-literal-2: can use expressions as property names', () => {
  const  createCandy = (type, description) => {
    return {
      tasty: true,
      type,
      [type.toUpperCase() + type.length]: description //type.toUpperCase() + type.length
