const noop = () => { } //una función que no hace nada
...
test('04_destructure-1: can be used to pull apart objects', () => {
  ...
  // ===Destructurado=== /*abrir un objeto y sacar propieddes directamente a variables*/
...
test('04_destructure-5: can alias destructured variables', () => {
  ...
  // const { city, state, zip } = getAddress() //TEST FALLARIA PORQUE SI ENCONTRARIA
  // const { city: c } = getAddress() //const { propiedadOriginal: nuevoNombre } = objeto
  const {
    city: c,
    state: s,
    zip: z
  ...
  expect(s).toBe('UT')
  expect(z).toBe(84115)
  expect(() => noop(city)).toThrow() //para comprobar que no hemos hecho const { city, state, zip } = getAddress()
  expect(() => noop(state)).toThrow()
  expect(() => noop(zip)).toThrow()
})

test('04_destructure-6: can destructure nested variables', () => { //desestructuración anidada.
  // Utilitzant desestructuració, crida `getAddress()` i crea les variables `lat` i `long`.

  // const {
  //   propiedadDentroDelObjeto: {
  //     propiedadInterior
  //   }
  // } = objeto
